  ...
  db.collection("exercises")
    ...
    .then((doc) => {
      if (doc && doc.data()) {
        ...
        let sessionDateOnly = `${sessionDateAndTime.toLocaleString("default", {
          month: "long",
        })} ${sessionDateAndTime.getDate()}, ${sessionDateAndTime.getFullYear()}`;
...
function populateWorkoutSessionHistory() {
  ...
  let workoutHistoryTemplate = document.getElementById(
  ...
  let sessionDateAndTime = new Date();
  // Split the string to get the date only
  let sessionID = `${sessionDateAndTime.toLocaleString("default", {
    month: "long",
  })} ${sessionDateAndTime.getDate()}, ${sessionDateAndTime.getFullYear()}`;
  ...
  db.collection("users")
    ...
    .then((allWorkouts) => {
      ...
      workouts.forEach((doc) => {
        ...
        workoutCard.getElementById(
          "weightHistory"
        ).innerHTML = `Weight <span id="metric">(lb)</span> ${weight}`;
        ...
        workoutCard
          ...
          .setAttribute("onclick", 'deleteWorkout("' + docID + '")');
...
/**
 * Deletes a workout from the database collection
 * @param id Workout ID
 */
